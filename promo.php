<?PHP
	...
	function show_slots($group_name, $group_link, $a, $ad_shedule, $horizon, $id) {
		...
		$count_slots = $db->query("SELECT datestamp from rabbit_posts where link NOT like 'https://vk.com%' and group_name='".$group_name."' and datestamp>='".date("Y-m-d", time()-3600*24*2)." 00:00:00'");
		while ($count_slot = $count_slots->fetch(PDO::FETCH_ASSOC)) {
			$c_slot.=$count_slot['datestamp'];
		}
		
		$r_slots = $db->query("select datestamp from rabbit_posts where deleted>0 and link not like 'https://vk.com%' and group_name='".$group_name."' and datestamp>='".date("Y-m-d", time()-3600*24*2)." 00:00:00' and id not in (select post_id from rabbit where post_type='adv')");
		while ($r_slot = $r_slots->fetch(PDO::FETCH_ASSOC)) {
			$red_slots.=$r_slot['datestamp'];
		}

		$b_slots = $db->query("select datestamp from rabbit_posts where deleted>0 and link not like 'https://vk.com%' and group_name='".$group_name."' and datestamp>='".date("Y-m-d", time()-3600*24*2)." 00:00:00' and id in (select post_id from rabbit where post_type='block')");
		while ($b_slot = $b_slots->fetch(PDO::FETCH_ASSOC)) {
			$black_slots.=$b_slot['datestamp'];
		}
		
		$g_slots = $db->query("select datestamp from rabbit_posts where deleted>0 and link not like 'https://vk.com%' and group_name='".$group_name."' and datestamp>='".date("Y-m-d", time()-3600*24*2)." 00:00:00' and id in (select post_id from rabbit where post_type='adv')");
		...
		$x = explode(";", $ad_shedule);
			for ($i = 0; $i < count($x); $i++) {
				...
				if ($t < time()+$horizon*3600) {
					...
					{
						if (!strstr($c_slot, date("Y-m-d H:i:s", $t))) $out.="&nbsp;<button class=\"btn btn-info shiny\" onclick=\"for (i=1;i<500;i++) {s = getElementById('sprut'+i); if (s && s.style.display == 'block') { var nameRadio = document.getElementsByName('fText'+i); for (var e = 0; e < nameRadio.length; e++) {  if (nameRadio[e].type === 'radio' && nameRadio[e].checked) fText = nameRadio[e].value;} var nameRadio = document.getElementsByName('fPic'+i); for (var e = 0; e < nameRadio.length; e++) {  if (nameRadio[e].type === 'radio' && nameRadio[e].checked) fPic = nameRadio[e].value;} var nameRadio = document.getElementsByName('fLink'+i); for (var e = 0; e < nameRadio.length; e++) {  if (nameRadio[e].type === 'radio' && nameRadio[e].checked) fLink = nameRadio[e].value;} var p = fPic+'|'+fLink+'|'+fText; if (strstr(p, 'false')) {alert('Выберите по одной картинке, ссылке и тексту!');return(false);} if (this.className=='btn btn-warning shiny') this.className='btn btn-info shiny'; else this.className='btn btn-warning shiny';ajax({
																   url:'get_ajax.php?slot=".urlencode(date("Y-m-d H:i:s", $t))."&act='+this.className+'&p='+p+'&group_id=".$id."',
																   method:'GET',
																   data: { },
							...
							$out.="&nbsp;<button class=\"btn btn-warning shiny\" onmouseout=\"var s=getElementById('".$div_id."'); if (s) s.style.display = 'none';\" onmouseover=\"var s=getElementById('".$div_id."'); if (s) { positionAt(this, s); s.style.display = 'block'; }\" onclick=\"ajax({
																   url:'get_ajax.php?slot=".urlencode(date("Y-m-d H:i:s", $t))."&act=info&group_id=".$id."',
																   method:'GET',
																   data: { },
...
<?PHP
	...
	$products = $db->query("SELECT id, title from products where prod_type='adv' $sql order by id");
	$prods = array();
    ...
    <div class="col-lg-12 col-xs-12 col-md-12">
        <div class="widget radius-bordered">
            <div class="widget-header bg-danger">
                <span class="widget-caption">Рекламные посты</span>
            </div>
            <div class="widget-body">
					<button class="btn btn-success shiny">Опубликован</button>
					<button class="btn btn-danger shiny">Ошибка</button>
					<button class="btn btn-purple shiny">Группа заблокирована</button>					
			...
			for ($j = 1; $j < count($prods)+1; $j++) {
				echo "<div id=sprut".$prods[$j-1]." style=\"display:none;\"><br><B>Теперь выберите составные части промо и временной слот:</B><br>";
				$s = file_get_contents('http://admin:cmyk7701@88.198.139.146/sprut1/show_product.php?pid='.$prods[$j-1]);
				echo $s;
				echo "</div>";
			}
			?>
			...
			<thead><tr><th><B>Группы</th><th></th><th></th><th></th><th></th><th></th><th align=center><B>Сегодня</b></th><th></th><th></th><th></th><th></th><th></th><th></th><th></th><th></th><th></th><th></th></tr></thead>
<?PHP
		...
		$m = $db->query("SELECT id as group_id, name as group_name, myworld_group as myword, ad_shedule, horizon, poster_id, group_admin_id from sprut_group ".$sql." order by name");
		...
		while($list = $m->fetch(PDO::FETCH_ASSOC)) {
			...
			echo "</td>";
				...
				echo "<td bgcolor=#eeeeee style=\"cursor: hand;width:30px;\" onclick=\"for (i=33;i<3156;i++) { var s = getElementById('row'+i+'_'+'".$i."'); if (s) { if (s.style.display == 'none') s.style.display = 'block'; else s.style.display = 'none';}} \"><B><font style=\"font-size:10px;color:#aaaaaa;\">".date("Y m.d", time()+($i)*3600*24)."</font></B></td><td nowrap><div id=row".$list['group_id']."_".$i." $disp>".show_slots($list['group_name'], $list['myword'], $i, $list['ad_shedule'], $list['horizon'], $list['group_id'])."</div></td>";
			}
			echo "</tr>";
		}					
?>					
