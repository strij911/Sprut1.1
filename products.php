...
<title>SPRUT 1.1 - Продукты</title>
</head>
    ...
    <div class="col-lg-12 col-xs-12 col-md-12">
        <div class="widget radius-bordered">
            <div class="widget-header bg-danger">
                <span class="widget-caption">Мои продукты</span>
            </div>
            <div class="widget-body">
				<table class="table table-bordered table-hover dataTable no-footer">
				<a href="create_prod.php" class="btn btn-danger"><i class="menu-icon glyphicon glyphicon-book"></i><span class="menu-text"> Создать новый продукт</span></a> <br>	<br>			
<?PHP
		
		if (get_userid() > 1) $usr = "user_id=".get_userid(); else $usr = '1=1';
		$u = $db->query("SELECT * FROM products where prod_type='adv' and $usr ORDER BY datestamp desc");
		while($list = $u->fetch(PDO::FETCH_ASSOC)) {
			if (!empty($_GET['pid']) && $list['id'] == $_GET['pid']) {
				echo '<tr><td colspan=3><font size=+1><B>'.$list['title'].'</B></td></tr><tr><td colspan=3><br></td></tr><tr><td valign=top colspan=3>';
				echo '<a name=active><form action=add_public.php>';
				echo "<a href=\"add_pic.php?pid=".$list['id']."\" class=\"btn btn-success\"><i class=\"menu-icon glyphicon glyphicon-book\"></i><span class=\"menu-text\"> Добавить картинку</span></a>";			
				echo '<p><div style="width: 1200px; height: 150px; overflow-y: scroll;">';

				echo show_pics($list['id']);
				
				echo "</div>";
				
				echo "<P><br><a href=\"add_link.php?pid=".$list['id']."\" class=\"btn btn-success\"><i class=\"menu-icon glyphicon glyphicon-book\"></i><span class=\"menu-text\"> Добавить ссылку</span></a>";
				echo '<p><div style="width: 1200px; height: 100px; overflow-y: scroll;">';
				echo show_links($list['id']);
				echo "</div>";
				
				echo "<P><br><a href=\"add_txt.php?pid=".$list['id']."\" class=\"btn btn-success\"><i class=\"menu-icon glyphicon glyphicon-book\"></i><span class=\"menu-text\"> Добавить текст</span></a>";			
				echo '<p><div style="width: 1200px; height: 150px; overflow-y: scroll;">';

				echo show_txt($list['id']);
				
				echo "</div><input type=hidden name=pid value=".$list['id'].">";
				//echo "<P><br><button type=\"submit\" class=\"btn btn-default shiny\" name=\"submit\" value=1>Создать промо</button>";
				echo '</form>';
				echo "</td><tr><td colspan=3><hr width=100%></td></tr>";
			} else {
				echo '<tr><td>'.show_img($list['id']).'</td><td width=20%><a href=products.php?pid='.$list['id'].'#active><B>'.$list['title'].'</B></a></td></tr>';
			}	
		}
?>					
				</table>
