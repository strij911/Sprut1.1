<?PHP
    ...
    if (!empty($_POST['title'])) {
		...
		$m = $db->query("INSERT INTO products (user_id, title, datestamp, prod_type) VALUES ('".get_userid()."', '".$_POST['title']."', '".date("Y-m-d H:i:s")."', 'vid')");
		$sid = $db->lastInsertId('products_id_seq');
		log_event("Продукт ".$_POST['title']." ($sid) успешно добавлен!");
		header('Location: products_vid.php?pid='.$sid);	
	} //if	
...
<title>SPRUT 1.1 - Добавление видео продукта</title>
</head>
<body>
